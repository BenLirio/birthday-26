...
export function updateUI() {
  ...
  Object.keys(friendTypes).forEach((type) => {
    ...
    if (btn) {
      btn.disabled =
        gameState.money < friendTypes[type].cost ||
        gameState.gameOver ||
        !gameState.gameStarted ||
        !gameState.mapSelected;
    }
  });
  ...
  if (statusEl) {
    if (!gameState.mapSelected) {
      ...
      if (
        remaining === 0 &&
        gameState.enemiesSpawned >= gameState.enemiesRemaining &&
        !gameState.gameOver
      ) {
        setTimeout(() => {
  ...
  if (friendsCodex) {
    ...
    for (let [type, data] of Object.entries(friendTypes)) {
      ...
      if (data.strong.length > 0) {
        strongIndicators = data.strong
          .map((enemy) => `<span style="color: #4CAF50;">${enemyTypes[enemy].icon}</span>`)
          .join(' ');
      }
      if (data.weak.length > 0) {
        weakIndicators = data.weak
          .map((enemy) => `<span style="color: #F44336;">${enemyTypes[enemy].icon}</span>`)
          .join(' ');
      }
    ...
    for (let [type, data] of Object.entries(enemyTypes)) {
      ...
      for (let [friendType, friendData] of Object.entries(friendTypes)) {
        if (friendData.strong.includes(type)) {
          counters.push(
            `<span style="color: ${friendData.color};">${friendType.charAt(0).toUpperCase()}</span>`,
          );
        }
...
export function isOnFriend(x: number, y: number) {
  return gameState.friends.some(
    (friend) => Math.sqrt((friend.x - x) ** 2 + (friend.y - y) ** 2) < 35,
  );
}
