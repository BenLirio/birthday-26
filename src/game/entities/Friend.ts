...
export class Friend {
  ...
  attack() {
    ...
    if (
      (this.stats.special === 'slow_only' || this.stats.special === 'conspiracy_only') &&
    ...
    if (this.stats.special.includes('wide') && this.upgrades.includes('wide')) {
      ...
      gameState.projectiles.push(
        new Projectile(this.x, this.y, this.target, this.stats.damage, this.type),
      );
    }
  }
  ...
  wideshotAttack() {
    ...
    for (let enemy of gameState.enemies) {
      ...
      if (distance <= effectiveRange) {
        gameState.projectiles.push(
          new Projectile(this.x, this.y, enemy, this.stats.damage, this.type),
        );
      }
    }
  }
  ...
  useSpecialAbility() {
    ...
    switch (this.stats.special) {
      ...
      case 'singing_stun':
        const singRange = this.upgrades.includes('harmony')
          ? this.stats.range + 50
          : this.stats.range;
